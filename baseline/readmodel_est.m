function [m,p,mss] = readmodel_est(pEst)
...
% output persistence;
p.b0 = pEst.b0;
p.b1 = pEst.b1; % b1 varies between 0.1 (extremely flexible) and 0.95(extremely persistent)

% policy passthrough (the impact of monetary policy on real economy); 
p.b2 = pEst.b2; % b2 varies between 0.1 (low impact) to 0.5 (strong impact)

% the impact of external demand on domestic output; 
p.b3 = pEst.b3; % b3 varies between 0.1 and 0.7

% the weight of the real interest rate and real exchange rate gaps in Monetary Conditions Index;
p.b4 = pEst.b4; % b4 varies from 0.3 to 0.8
...
p.a1 = pEst.a1; % a1 varies between 0.4 (low persistence) to 0.9 (high persistence)

% passthrough of marginal costs to inflation (the impact of rmc on inflation); 
p.a2 = pEst.a2; % a2 varies between 0.1 (a flat Phillips curve and a high sacrifice ratio) to 0.5 (a steep Phillips curve and a low sacrifice ratio)

% the ratio of domestic costs in firms' aggregate costs
p.a3 = pEst.a3; % a3 varies between 0.9 (for a relatively more closed economy) to 0.5 (for a relatively more open economy)
...
% policy persistence; 
p.g1 = pEst.g1; % g1 varies from 0 (no persistence) to 0.8 ("wait and see" policy)

% policy reactiveness: the weight put on inflation by the policy-makers 
p.g2 = pEst.g2; % g2 has no upper limit but must be always higher then 0 (the Taylor principle)

% policy reactiveness: the weight put on the output gap by the policy-makers 
p.g3 = pEst.g3; % g3 has no upper limit but must be always higher then 0
...
p.e1 = pEst.e1; % setting e1 equal to 0 reduces the equation to the simple UIP 
...
p.rho_D4L_CPI_TAR = pEst.rho_D4L_CPI_TAR; 
...
p.rho_DLA_Z_BAR   = pEst.rho_DLA_Z_BAR;
p.rho_DLA_GDP_BAR = pEst.rho_DLA_GDP_BAR;
p.rho_RR_BAR      = pEst.rho_RR_BAR;
p.rho_RR_RW_BAR   = pEst.rho_RR_RW_BAR;

% persistence in foreign output gap 
% L_GDP_RW_GAP = rho_L_GDP_RW_GAP*L_GDP_RW_GAP{-1} + SHK_L_GDP_RW_GAP;
p.rho_L_GDP_RW_GAP = pEst.rho_L_GDP_RW_GAP;

% persistence in foreign interest rates and inflation
% RS_RW = rho_RS_RW*RS_RW{-1} + (1-rho_RS_RW)*(RR_BAR + DLA_CPI_RW) + SHK_RS_RW;
p.rho_RS_RW      = pEst.rho_RS_RW;
p.rho_DLA_CPI_RW = pEst.rho_DLA_CPI_RW;
